...
int codon_stop(char *adn,int *j ){
	...
	for(i=0;i<strlen(adn);i=i+3){
		if (strncmp(adn+i,"TAG",3)==0 || strncmp(adn+i,"TGA",3)==0 || strncmp(adn+i,"TAA",3)==0){
			pos=i;
			break;
		}
	}
...
char *suprimer_duplic(char *adn,int taille){
	...
	char *trip=(char *)malloc(3*sizeof(char));//is not nessary //look at line 140
	for(int i=0;i<taille;i=i+3){
		strncpy(trip,adn+i,3);
		if (strstr(new_codones,trip)==NULL){// use adn+i and strnstr
