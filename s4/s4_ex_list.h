...
List *suprimer_precedent_list(List *li){
    Element *e=li->premier;
    if(estvide) return li;
    if (!estvide){
        li->nb_elm--;
...
void affiche_list(List *li){
    ...
    while(current!=NULL){
        ...
        ecrire_fiche(ob);//pour affiche les info de data et la fonction affiche les info ca depend de data;
        current=current->suivant;
    }
}
