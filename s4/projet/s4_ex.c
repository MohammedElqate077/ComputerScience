...
void affiche_list(List *li){
    Element *current=li->premier;
    while(current!=NULL){
        objet *obj=current->data;
       
        ecrire_objet(obj);//pour affiche les info de data et la fonction affiche les info ca depend de data;
        current=current->suivant;
    }
    ...
    while( choi!=8){
        ...
        switch(choi){
            ...
            */
            case 8 :
            	system("clear");
            	printf("\t\t ** affichage de la objet **\n");
                affiche_list(li);break;
            case 9 :
                exit(0);
            default : 
            	system("clear");
                puts("le choi incorect : saisire le bon choi \n");
                goto start;
        }
        if (choi==1) puts("\n  la list est crea\n ");
    }
