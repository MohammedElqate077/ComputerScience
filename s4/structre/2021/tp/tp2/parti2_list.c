...
void inser_dec_poly(Poly *po,Objet ob){
	...
	//Objet ob;
	char FIN='k';	
	printf("enter < F > if you want fin inserer\n");
	do{
	FIN=getchar();
	ob=get_objet();
	if(est_vide(po)){
...
void inser_liste_poly(List_poly *li){
	List_poly *now=li;	
	Poly *po=create_poly();
	Objet ob;
	if(li==NULL){
		li=(List_poly*)malloc(sizeof(List_poly));
		li->suiv=NULL;
		li->num=1;
		li->P_po=create_poly();
		inser_dec_poly(li->P_po,ob);
		return;
	}
	int i=2;
	while(now->suiv!=NULL){
	...
	now->suiv=(List_poly*)malloc(sizeof(List_poly));
	now->suiv->P_po=create_poly();
	now->suiv->suiv=NULL;
	now->suiv->num=i;
	inser_dec_poly(now->suiv->P_po,ob);
}
...
int main(){
	...
	do{
		...
		switch (choi){
			case 1:
					inser_liste_poly(li);break;
					//create_poly();break;
			case 2:

					//ob=get_objet();
					printf("enter le nb de polynome dont ajoute monome\n");scanf("%d",&n);
					i=1;	
					while(li!=NULL && i<n){ li=li->suiv; i++; }
					inser_dec_poly(li->P_po,ob);
					break;
			case 3:
					//ob=get_objet();
