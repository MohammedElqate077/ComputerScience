...
void Vector::add(String element) {
  ...
  if (sizeVector > 1) {  // for reallocation of tabString;
    ...
    for (int i = 0; i < sizeVector; i++) {
      ...
      ptr = tempString[i].get();
      free(ptr);
      ptr = nullptr;
      tempString[i].set((void*)ptr);
    }
    delete[] tempString;
    tempString = nullptr;
  } else {
...
String Vector::get(int index) {
  ...
  return temp;*/
   if(index >= sizeVector)
      std::cout << "this index not under size" << std::endl;
   else 
...
String Vector::remove(int index) {
  ...
  while (i < sizeVector - 1) {
     ...
     ptr = nullptr;
     tabString[i].set(ptr);
     */ //  I test this after while << delete tabString; >>
     i++;
  }
  delete []tabString;
  sizeVector--;
...
void Vector::print() {
  if (sizeVector == -1) std::cout << "empty vector " << std::endl;
  for (int i = 0; i <= sizeVector; i++) {
    tabString[i].print();
  }
}
