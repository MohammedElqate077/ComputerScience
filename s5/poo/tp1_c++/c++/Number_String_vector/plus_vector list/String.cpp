...
void String::set(char *chaine) {
  ...
  this->chaine = nullptr;
  this->chaine = new char[strlen(chaine) + 1];
  strncpy(this->chaine, chaine, strlen(chaine)+1);
}
...
String &String::operator=(const String &chaine) {
  if(chaine.chaine == nullptr);/*{
     char temp[] = "";
     this->set(temp);
     std::cout << "your element is empty (NULL) , so your object has affected by emty string" << std::endl;
     return *this;
  }*/
  /*this->chaine = new char[strlen(chaine.chaine) + 1];
  strcpy(this->chaine, chaine.chaine);
  */
  this->set(chaine.chaine);
